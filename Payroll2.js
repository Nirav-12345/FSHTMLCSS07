 ...
 for(const employeePayRollData of empPayRollList) {
    ...
    innerHTML=`${innerHTML}
        ...
        <tr>
            ...
            <td>${stringfyDate(employeePayRollData._startDate)}</td>
            <td><button onclick=remove(${employeePayRollData._id})>Remove</button>
            <td><button onclick=update(${employeePayRollData._id})>Update</button>
            </td>
        </tr>
        `;
    }
...
const update=(node) =>{
    ...
    window.location.replace(siteproperties.add_emp_payroll);
}